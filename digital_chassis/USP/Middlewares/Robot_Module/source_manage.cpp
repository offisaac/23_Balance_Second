...
void C_SourceManage_Classdef::Set_ChargeCurrent(void)
{
	...
	else
	{
		...
		/* 充电电流限幅，安全起见先限幅为输出2A */
		chargeCurrent = std_lib::constrain(chargeCurrent, 0.0f, 2.0f);
	}
 ...
 * @note 
 * @param None
 * @retval 电容电压比归一化后的百分数（0-1）
 * @author Lingzi_Xie
 */
float C_SourceManage_Classdef::Get_EnergyPercentage(void)
{
	return voltage.vol_Out / CAP_VOLTMAX;
}
