...
#define ORDER_START_MOTOR 0x82 //  返回空值，不用处理
...
MotorMF9025v2Classdef::MotorMF9025v2Classdef(uint8_t _id)
{
  ...
  canSendStruct.ID = getTransmitId();
  canSendStruct.DLC = 8;
}
/**
 ...
 * 
 * @param id 要检查的canID
 * @return 若传参为电机id则返回true，否则返回false
 */
bool MotorMF9025v2Classdef::CheckID(uint16_t id)
{
  return id == this->getTransmitId();
}
 ...
 */
void MotorMF9025v2Classdef::writeEncoderOffset(uint16_t _offset)
{
 ...
 */
void MotorMF9025v2Classdef::update(uint8_t can_rx_data[])
{
...
{
  static float temp_angle = 0, last_temp_angle = 0;
  ...
  if (angle_offset_state)
  {
    ...
    temp_angle = dataPool.encoder * RATIO_ENCODER_TO_ANGLE;
    if (temp_angle - last_temp_angle > 180)
    {
      dataPool.round_cnt--;
      dataPool.totalAngleLocal += temp_angle - last_temp_angle - 360;
    }
    else if (temp_angle - last_temp_angle < -180)
    {
      dataPool.round_cnt++;
      dataPool.totalAngleLocal += temp_angle - last_temp_angle + 360;
    }
    ...
    {
      dataPool.totalAngleLocal += temp_angle - last_temp_angle;
    }
  }
  else
  {
