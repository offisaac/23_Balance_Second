...
extern "C"
{
    ...
    public:
        ...
        uint16_t getTransmitId() const { return REC_ID_INIT() + dataPool.motorId; }
        int16_t MAX_CURRENT() const { return 2000; }
        void update(uint8_t can_rx_data[]); //电机返回值更新函数
        bool CheckID(uint16_t id);        //检查CAN消息id是否为电机id
