...
public:
     ...
     * 示例：若1-3的id设置开关均为off，则变量构造函数传入Motor_GM6020 motor(0, 0, 0);
     *      若1为ON，2为OFF，3为ON，则变量构造函数传入Motor_GM6020 motor(1, 0, 1);
     */
    Motor_GM6020(bool id1, bool id2, bool id3) : MotorSpeed((uint8_t)id1 + 2 * (uint8_t)id2 + 4 * (uint8_t)id3){}
    virtual void update(uint8_t can_rx_data[])
    {
        this->update_angle(can_rx_data);
        this->update_speed(can_rx_data);
