/**
	...
	*  2022-07-13		Maohui	  6.0.0		添加自瞄框、平衡步状态和敌方血条的UI,解决整数、浮点数无法显示的bug
	...
	特别注意要用最新的裁判系统客户端，旧版有点问题
		...
		目前参考的是裁判系统串口协议附录V1.2 2021-04-30
		如有问题，请参考《RM2021裁判系统用户接口协议附录V1.2》以及SRML库中的referee_README.md
...
void referee_Classdef::HP_UI(uint8_t ID, uint16_t start_x, uint16_t start_y, uint16_t length, uint16_t width, colorType_e _color, uint8_t enable_cnt)
{
	...
	memcpy(data_pack, (uint8_t *)int_drawing(0, _operate_type, start_x - 20, start_y + width / 2, 10, 2, _color, (int32_t)ID, hp_name), DRAWING_PACK);
	...
	if (robot_client_ID.robot_where)
	{
		...
		memcpy(&data_pack[DRAWING_PACK * 3], (uint8_t *)int_drawing(0, _operate_type, start_x + length + 20, start_y + width / 2, 10, 2, _color, HP, hp_name), DRAWING_PACK);
	}
	else
	{
		if (ID > 0 && ID <= 6)
...
void referee_Classdef::Draw_Fri_State(uint8_t fri_flag, uint16_t center_x, uint16_t center_y)
{
	...
	if (enable_cnt)
	{
		Draw_Char(1, center_x, center_y, fri_name, fri_str, sizeof(fri_str), 15, YELLOW, ADD_PICTURE);
	}
	else
	{
	}
	...
	if (enable_cnt > 0)
	{
		Draw_Char(1, center_x + 80, center_y, fri_name, fri_str2, sizeof(fri_str2), 15, _color, ADD_PICTURE);
		enable_cnt--;
	}
	else
	{
		Draw_Char(1, center_x + 80, center_y, fri_name, fri_str2, sizeof(fri_str2), 15, _color, MODIFY_PICTURE);
	}
...
void referee_Classdef::Draw_Cap_Energy(float current_volt, float max_volt, float min_volt, uint8_t enable_cnt, uint16_t center_x, uint16_t center_y)
{
	...
	//电容余电进度条方框
	static uint8_t cap_name4[] = "ct4";
	//电容余电进度条填充
	static uint8_t cap_name5[] = "ct5";
	...
	if (enable_cnt)
	{
		//绘制电容余量字符串，需要多次绘制
		_operate_type = ADD_PICTURE;
		Draw_Char(1, center_x, center_y, cap_name1, cap_str, sizeof(cap_str), 15, YELLOW, _operate_type);
		memcpy(&data_pack[DRAWING_PACK * 2], (uint8_t *)rectangle_drawing(0, _operate_type, center_x-1, center_y - 91, 203, 22, 2, PURPLE, cap_name4), DRAWING_PACK);
	}
	else
		_operate_type = MODIFY_PICTURE;

	memcpy(data_pack, (uint8_t *)null_drawing(0, cap_name3), DRAWING_PACK);
	// memcpy(data_pack, (uint8_t *)float_drawing(0, _operate_type, center_x + 20, center_y - 80, 15, 2, _color, current_volt, cap_name3), DRAWING_PACK);
	memcpy(&data_pack[DRAWING_PACK], (uint8_t *)int_drawing(0, _operate_type, center_x, center_y - 40, 15, 2, _color, volt_proportion, cap_name2), DRAWING_PACK);
	
	memcpy(&data_pack[DRAWING_PACK * 3], (uint8_t *)line_drawing(0, _operate_type, center_x, center_y - 80, center_x + 200*volt_proportion/100, center_y - 80, 20, _color, cap_name5), DRAWING_PACK);
...
void referee_Classdef::Draw_Boost(uint8_t boost_flag, uint16_t center_x, uint16_t center_y, uint16_t line_width, colorType_e _color)
{
	...
	static uint8_t enable_cnt[2] = {6, 0};
	static uint8_t boost_name1[] = "bs1";
	static uint8_t boost_name2[] = "bs2";
	static uint8_t cap_str[] = "cap:";
	static uint8_t cap_str1[] = "OFF";
	static colorType_e color;

	if (enable_cnt[0])
	{
		Draw_Char(1, center_x, center_y, boost_name1, cap_str, sizeof(cap_str), 15, YELLOW, ADD_PICTURE);
		Draw_Char(1, center_x + 80, center_y, boost_name2, cap_str1, sizeof(cap_str1), 15, RED, MODIFY_PICTURE);
	}
	else
	{
	}
	...
	{
		enable_cnt[0]--;
		Draw_Char(1, center_x + 80, center_y, boost_name2, cap_str1, sizeof(cap_str1), 15, color, ADD_PICTURE);
	}
	else
	{
		Draw_Char(1, center_x + 80, center_y, boost_name2, cap_str1, sizeof(cap_str1), 15, color, MODIFY_PICTURE);
	}
...
void referee_Classdef::Draw_Auto_Lock_Range(uint8_t auto_flag, uint8_t auto_mode, uint16_t center_x, uint16_t center_y, uint16_t length, uint16_t width, uint16_t line_width)
{
	...
	static uint8_t auto_mode_str1[] = "Rune";
	static uint8_t auto_mode_str2[] = "Anti";
	static uint8_t auto_mode_str3[] = "Rotate";
	static uint8_t auto_mode_str4[] = "Sentry";
	...
	if (enable_cnt[2])
	{
		...
		case 2:
			...
			break;
		case 3:
			Draw_Char(1, center_x + 15 - 0.5 * length, center_y - 10 + 0.5 * width, auto_mode1, auto_mode_str3, sizeof(auto_mode_str3), 20, YELLOW, _operate_type);
			break;
		case 4:
...
void referee_Classdef::Draw_Balance_Stop_Erea(uint8_t _layer, uint8_t enable_cnt, float _current_speed, float _max_speed, float height, uint16_t scale)
{
	...
	_current_speed = std_lib::constrain(_current_speed, 0.f, _max_speed);

	float scale_up = _current_speed / _max_speed * scale; //区域上限位
	float scale_down = scale_up - height;									//区域下限位
	scale_up = std_lib::constrain(scale_up, 0.f, 1.f);
	scale_down = std_lib::constrain(scale_down, 0.f, 1.f);
...
void referee_Classdef::Draw_Rotation_Crash(uint8_t _layer, uint16_t center_x, uint16_t center_y, bool is_crash)
{
	...
	tips_delay--;
	tips_delay = std_lib::constrain(tips_delay, (uint8_t)1, (uint8_t)250);
}
...
void referee_Classdef::Draw_WheelBipe_Posture(uint8_t _layer, uint8_t enable_cnt, uint16_t center_x, uint16_t center_y, float b1, float b2, float b3, float _f_angle, float _b_angle, float bipe_endx, float bipe_endy, float _scale, colorType_e _color)
{
	...
	line_b1_startx = std_lib::constrain((uint16_t)(center_x - b1 / 2 * _scale), (uint16_t)0, (uint16_t)1920);
	line_b1_starty = std_lib::constrain(center_y, (uint16_t)0, (uint16_t)1080);
	line_b2f_endx = std_lib::constrain((uint16_t)(line_b1_startx + b1 * _scale + b2 * cosf(_f_angle) * _scale), (uint16_t)0, (uint16_t)1920);
	line_b2f_endy = std_lib::constrain((uint16_t)(center_y - b2 * sinf(_f_angle) * _scale), (uint16_t)0, (uint16_t)1080);
	line_b2b_endx = std_lib::constrain((uint16_t)(line_b1_startx + b2 * cosf(_b_angle) * _scale), (uint16_t)0, (uint16_t)1920);
	line_b2b_endy = std_lib::constrain((uint16_t)(center_y - b2 * sinf(_b_angle) * _scale), (uint16_t)0, (uint16_t)1080);
	line_b3_endx = std_lib::constrain((uint16_t)(center_x + bipe_endx * _scale), (uint16_t)0, (uint16_t)1920);
	line_b3_endy = std_lib::constrain((uint16_t)(center_y + bipe_endy * _scale), (uint16_t)0, (uint16_t)1080);
...
// 	//绘制电容余量字符串，需要多次绘制
// 	_operate_type = ADD_PICTURE;
// 	Draw_Char(1, center_x, center_y, cap_name1, cap_str, sizeof(cap_str), 15, WHITE, _operate_type);
// }
// else
// 	_operate_type = MODIFY_PICTURE;

// memcpy(data_pack, (uint8_t *)float_drawing(0, _operate_type, center_x + 20, center_y - 80, 15, 2, _color, current_volt, cap_name3), DRAWING_PACK);
// memcpy(&data_pack[DRAWING_PACK], (uint8_t *)int_drawing(0, _operate_type, center_x + 20, center_y - 40, 15, 2, _color, volt_proportion, cap_name2), DRAWING_PACK);
// pack_send_robotData(Drawing_2_ID, robot_client_ID.client, (uint8_t *)data_pack, DRAWING_PACK * 2);
