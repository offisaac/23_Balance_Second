...
float myPID::Adjust()
{
  ...
  if(std::abs(Ki) > std::numeric_limits<float>::epsilon()){
    ...
    /*Constrain*/
    integral_e = std_lib::constrain(integral_e, -fabsf(I_Term_Max/Ki), fabsf(I_Term_Max/Ki));
  }
  else{
    integral_e = 0;
  }
  ...
  { 
    ...
    /*Constarin*/
    I_Term = std_lib::constrain(I_Term, -fabsf(I_Term_Max), fabsf(I_Term_Max));
  }
  else{
    /*Clear*/
    I_Term = 0;
  ...
  Out = std_lib::constrain((P_Term + I_Term), -fabsf(PI_Term_Max) , fabsf(PI_Term_Max)) + D_Term;
  
  /* Constarin */
  Out = std_lib::constrain(Out, -fabsf(Out_Max), fabsf(Out_Max));
